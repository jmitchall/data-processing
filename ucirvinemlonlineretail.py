...
def get_retail_data(id=352):  # default = 352 retail Data Set
    online_retail = fetch_uc_irvine_ml_data(id)
    # metadata
    ...
    if online_retail.data.targets is None:
        y = pd.DataFrame()  # empty data frame
    else:
        y = online_retail.data.targets.copy()
...
if __name__ == '__main__':
    ...
    # fetch the dataset
    data_frame, X, Y, metadata, variables = get_retail_data()
    data_frame.to_csv('online_retail.csv', sep=",", index=False, header=False)
    diagnostic_data = pd.read_csv('online_retail.csv', header=None)
    print('X Columns', X.columns)
    print('Y columns', Y.columns)
    with open('online_retail_metadata.json', 'w') as jsonFile:
        json.dump(metadata, jsonFile, indent=4, sort_keys=True)

    variables.to_csv('online_retail_variables.csv', sep=",", index=False, header=False)
